...
layout(binding = 1) uniform sampler2D framebuffer_Normal;
layout(binding = 2) uniform sampler2D framebuffer_Albedo;
...
void main() {
  vec3 fragmentDiffuse = vec3(0);
	...
	vec3 lightColor = fragmentDiffuse * 0.15;
  ...
  //       if(distanceToLight > pointLights[i].radius * 2)
  //           continue;
	...
	// 	lightColor += (diffuseColor + specularColor) * attenuation;
	// }
